...
export class RegisterComponent implements OnInit, OnDestroy {
  ...
  show: boolean = false;
  roles = [
    new Role(RoleId.ADMIN, RoleTitle.ADMIN),
    new Role(RoleId.CUSTOMER, RoleTitle.CUSTOMER),
  ];
  minorCutOff = Date.parse(
  ...
  ngOnInit() {
    ...
    this.buildForm();
    this.roleSubscription = this.registerForm
      .get('role')
      .valueChanges.pipe(
        tap((role: string) => {
          if (role == 'Admin') {
            this.registerForm
              ...
              .setValidators([
                Validators.required,
                ValidationService.adminEmailValidator,
              ]);
          }
          if (role == 'Customer') {
            this.registerForm
  ...
  buildForm() {
    this.registerForm = this.formBuilder.group({
      ...
      confirmPassword: [
        '',
        [Validators.required, ValidationService.confirmPasswordValidator],
      ],
  ...
  generateUser() {
    ...
    if (this.registerForm.get('phone').value) {
      this.register.phone = this.registerForm
        .get('phone')
        .value.replace(/[()-\s]/g, '');
    }
    if (this.registerForm.get('dob').value) {
      this.register.dob = this.registerForm.get('dob').value;
    }
